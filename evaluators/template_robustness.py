...
class Template_Checker(object):
    ...
    def calculate_sample_step_score(self, predict_sample_path, gt_sample_path, step_idx, total_score_cal, tep, tgt, step_num, data_type, module):
        # Table 3, consider to merge with Table 2
        # ic(tep, tgt)
        text_input = torch.cat([clip.tokenize(tep)]).to(self.device)
        image = Image.open(os.path.join(predict_sample_path, f"step_{step_idx}_bridge.png"))
    ...
    def eval_template(self, taskpath, template, data_type, module):        
        total_score_cal = {"vplan-vgt-clip-score": 0, "tplan-tgt-clip-score": 0, "tplan-vgt-clip-score": 0, "vplan-tgt-clip-score": 0}
        ...
        for task_idx in tqdm(range(task_num)):
            ...
            for step_idx in range(1, step_num+1):
                # TODO: _bridge_caption => _bridge_tplan after generate bridge tplan
                tep = self.get_content(predict_sample_path, step_idx, "_bridge_caption")
                tep = f"Step {step_idx}: {tep}"
                tgt = self.get_content(gt_sample_path, step_idx, "")
                total_score_cal = self.calculate_sample_step_score(predict_sample_path, gt_sample_path, step_idx, total_score_cal, tep, tgt, step_num, data_type, module)
        ...
        csv_line = [self.task_name] + [module] + [template] + [total_score_cal["vplan-vgt-clip-score"]] + [total_score_cal["tplan-tgt-clip-score"]] + [total_score_cal["vplan-tgt-clip-score"]] + [total_score_cal["tplan-vgt-clip-score"]]
        return csv_line
...
if __name__ == "__main__":
    ...
    for data_type in ["wikihow", "recipeqa"]:
        ...
        out_path = os.path.join(opt.image_root, "template_eval_output")
        os.makedirs(out_path, exist_ok=True)
        with open(os.path.join(out_path, "all_template_metric.csv"), 'w') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quoting=csv.QUOTE_MINIMAL)
            head_line = ['task_name', 'module', 'template', 'vplan-vgt-clip-score', 'tplan-tgt-clip-score', 'vplan-tgt-clip-score', 'tplan-vgt-clip-score']
            ...
            for task_name in ["tgt-u-plan"]:
                ...
                for module in ["t2i-bridge", "i2t-bridge"]:
                    # template_list = [f"t2i-{i}" for i in range(8)] if task_name in ["tgt-u-plan"] else [f"i2t-{i}" for i in range(8)]
